...
namespace datamining_lab1::one_rule
{
  ...
  public:
    ...
    {
      // I can do this check, but this check is going to be the same as in any hardened c++ library
      // for bounds checking. if (position < value.size()) [[unlikely]]
      //   {
      //     return {};
      //   };
      return valueable_data_.at(*std::next(value_collection.cbegin(), position));
    }
    ...
    [[nodiscard]] constexpr inline auto getPosition() const { return position; }
    [[nodiscard]] constexpr inline const auto& getValuableData() const { return valueable_data_; }
  };

  // additional deduction guide...
  template <std::ranges::bidirectional_range RangeT> model(RangeT range)
      -> model<typename decltype(std::declval<std::iter_value_t<std::ranges::iterator_t<RangeT>>>()
                                     .getValue())::value_type,
               decltype(std::declval<std::iter_value_t<std::ranges::iterator_t<RangeT>>>()
                            .getExpectedResult())>;
             ...
             and Con_HasgetExpectedResult<std::iter_value_t<std::ranges::iterator_t<RangeT>>>
  {
    ...
    std::vector<std::map<underneeth_data_type, std::map<underneeth_expected_result_type, std::size_t>>>
        ...
        for (const auto& [row_key, row_map] : tables[parameter_index])
          {
            std::pair<underneeth_expected_result_type, std::size_t> pair_with_max
                = *std::ranges::max_element(row_map, [](auto const& pair1, auto const& pair2) {
            ...
            // auto incorrect
            //     = row_map | std::views::keys
            //       | std::views::filter([key_with_max = pair_with_max.first](auto const& val) {
            //           return val != key_with_max;
            //         });
            // error_of_table[parameter_index]
            //     += std::ranges::fold_left(incorrect, std::size_t{}, std::plus{});
          }
      }

    std::size_t best_table_index = std::distance(
        error_of_table.cbegin(), std::min_element(error_of_table.cbegin(), error_of_table.cend()));

    fmt::println("error_of_table: {} , best_table_index: {} , amount_of_parameters {}",
    error_of_table, best_table_index, amount_of_parameters);

    std::size_t position = best_table_index;
    std::map<underneeth_data_type, underneeth_expected_result_type> valueable_data;
    ...
    return model<underneeth_data_type, underneeth_expected_result_type>(std::move(position),
                                                                   std::move(valueable_data));
  };
